...
## Copilot and agent integration: Lakehouse tables

This is a focused quick-reference for the tables that Copilot topics, Power Automate flows, and the data agent interact with directly. Tables that are used exclusively by the notebooks or reports are documented in the full semantic-model inventory below.

| Lakehouse Table | Purpose for Copilot / Agent | Copilot / Flow / Agent consumer | Key Columns |
|---|---|---|---|
| `rs_approved_regions` | Authoritative list of approved residency region codes. Read by the compliance notebook to gate the eligibility table; Copilot checks this indirectly via the eligibility table. | Notebook: `notebook_fabric_function_sov_compliance_checks_new` (eligibility gate); flow `RetrieveSovDashboardInfo` (via `dp_copilot_purview_residency_change_eligibility`) | `region_name`, `region_code`, `approved`, `notes` |
| `dbo.dp_dataproduct_fullnameclassification_current` | Per-product flag indicating whether any asset column is classified as `MICROSOFT.PERSONAL.NAME`. Used by the CC/PII eligibility flow and data agent to reason about PII risk. | Flow `Retrieve_CCPII_Eligibility`; topic `Apply_PII_CC_Investigate_Tag`; data agent (CC/PII scenarios) | `DataProductId`, `DataProductDisplayName`, `HasFullNameClassification`, `FullNameColumnCount` |
| `dbo.dp_compute_inventory_current` | Current compute resource inventory per data product (Azure Resource Graph). Used by the data agent and CC/PII flow to evaluate whether sensitive-data products have confidential compute mapped. | Flow `Retrieve_CCPII_Eligibility`; topic `Apply_PII_CC_Investigate_Tag`; data agent | `DataProductId`, `ComputeResourceId`, `ComputeName`, `Location`, `IsConfidential` |
| `dp_copilot_purview_residency_change_eligibility` | Deterministic eligibility gate for Copilot-driven Purview residency updates. Precomputes whether each product is eligible, what the target region is, and what action to take. This is the primary table the Copilot residency workflow depends on. | Flow `RetrieveSovDashboardInfo` (outputs `EligibleResidencyUpdateCount/Products/Details`); topic `Purview Residency Glossary Update Workflow`; semantic model alias `Copilot Purview Residency Change Eligibility` | `DataProductID`, `DataProductDisplayName`, `AzureResidencyCode`, `ExpectedCurrentResidencyCode`, `TargetResidencyCode`, `EligibleForResidencyUpdate`, `EligibilityReason`, `SuggestedPurviewResidencyAction` |
